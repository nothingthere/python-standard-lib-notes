...
**** 常用操作
     ...
     #+BEGIN_SRC python :eval never
       ...
       c.most_common()[:-n - 1:-1]     # 获取个数最少的N个元素
       +c                              # 将个数<=0的元素去除
     #+END_SRC
...
* weakref - 弱引用
  ...
  #+BEGIN_QUOTE
  Python采用计数制进行垃圾回收，即对象被引用的次数为0时被回收。
  #+END_QUOTE
  
  使用weakref模块可为对象创建弱引用。
  
  一个对对象的弱引用并不足够使对象保持存活：当对象的被引用只剩弱引用时，
  就可以被回收，释放所占内存。但是，直到该对象被真正销毁前，即时没有强
  引用，弱引用也可返回该对象。
  
  弱引用的主要用途为实现对大型对象缓存和映射的引用。因为只需大型对象存
  在于缓存或映射中，不需保持存活。
  
  比如，如需将多个大型二进制图像对象与名称映射，如果使用Python字典将名
  称映射为图像对象，或图像对象映射为名称，由于图像对象为字典的键或值，
  会一直保持存活。这时，可使用weakref模块的WeakKeyDictionary和
  WeakValueDictionary，这样就图像对象就不会一直存活。比如将图像对象作
  为WeakValueDictionary的value值，当对图像对象的引用只剩该弱映射引用时，
  就可被垃圾回收，在映射中的节点也被删除。
  
  WeakKeyDictionary和WeakValueDictionary中，可设置回调函数，当弱引用对
  象被删除时执行。WeakSet实现了set类型接口，对set对象的元素进行弱引用。
  
  使用finalize可直接为对象注册函数，当被回收后调用。比直接在原生弱引用
  对象上声明回调函数更简单，因为即时被引用对象被回收，finalize对象依然
  存在。
  
  对绝大多数程序而言，上面的3个弱引用容器和finalize就够用，没必要手动
  创建弱引用机制。weakref模块暴露底层机制只为高级用法。
  
  不是所有对象都支持被弱引用。支持的有：类实例、Python函数（非C函数）、
  实例方法、set、frozenset、一些文件对象、生成器（generator）、类型对
  象、socket、array、deque、正则匹配对象和代码对象。
  
  如list和dict这样的内置类型不能直接支持弱引用，不过可通过子类实现：
  
  #+BEGIN_SRC python :eval never
    class Dict(dict):
        pass

    obj = Dict(read=1, green=2, blue=3)  # 此对象支持弱引
  #+END_SRC
  
  其他如tuple和int的内置类型即使通过子类也无法支持弱引用。
  
** 函数
*** class weakref. *ref* (object[,callback])
    返回一个弱引用对象。如果被引用对象依然存活，调用此弱引用对象返回被
    引用对象；如果被引用对象不再存活，调用此引弱用对象返回None。
    
    如果提供CALLBACK参数且不为None，当返回的弱引用对象将被终结时调用，
    只有一个表示弱引用对象的参数。调用后，返回的弱引用对象不再存在。
    
    如果允许为相同对象创建弱引用，回调函数会从最后添加的开始调用。
    
    如果被引用对象相等，则返回的弱引用对象也相等，不受回调函数影响。当
    某个被应用对象被删除后，只有引用相同对象的弱引用对象才相等。
    
    #+BEGIN_SRC python :session
      from weakref import ref
      s = {1, 2, 3}

      def about_to_del1(obj):
          print('1：弱引用对象：{} 被删除'.format(obj))

      def about_to_del2(obj):
          print('2：弱引用对象：{} 被删除'.format(obj))

      x = ref(s, about_to_del1)
      y = ref(s, about_to_del2)
    ...
    #+BEGIN_SRC org
    - >>> >>> >>> ... ... >>> >>> ... ... >>> >>> >>> >>> >>> True
    - >>> 2：弱引用对象：<weakref at 0x7fb73b013458; dead> 被删除
    - 1：弱引用对象：<weakref at 0x7fb73b013548; dead> 被删除
    - 被引用对象不再存活后：
    - <weakref at 0x7fb73b013548; dead> <weakref at 0x7fb73b013458; dead>
    - None None
    #+END_SRC
...
**** __callback__
     自读属性，返回弱引用对象创建时的CALLBACK参数。如果没有CALLBACK参
     数，或被引用对象不再存在，此属性值为None。

     #+BEGIN_SRC python :session
       from weakref import ref
       s = set((1, 2, 3))

       def callback(ref_obj):
           print(ref_obj)

       x = ref(s, callback)
       print(x.__callback__)
       del s
       print(x.__callback__)
     #+END_SRC
     ...
     #+RESULTS:
     #+BEGIN_SRC org
     - <weakref at 0x7f2a31754598; dead>
     - >>> ... ... >>> >>> <function callback at 0x7f2a3103b510>
     - <weakref at 0x7f2a317544a8; dead>
     - None
     #+END_SRC

*** weakref. *proxy* (object[,callback])
    使用弱引用返回OBJECT的代理对象。返回值的类型取决与OBJECT是否为
    callble类型为ProxyType或CallableProxyType。不管被引用对象是否可哈
    希，返回的代理对象都不可哈希。
    ...
    #+BEGIN_SRC python :eval never
      ...
      x = proxy(s, about_to_del1)
      y = proxy(s, about_to_del2)
...
*** weakref. *getweakrefs* (obj)
    ...
    #+BEGIN_SRC org
    - >>> []
    - >>> [<weakref at 0x7f72f0242598; to 'set' at 0x7f72f0220128>]
    - >>> [<weakref at 0x7f72f0242598; to 'set' at 0x7f72f0220128>, <weakproxy at 0x7f72f0242728 to set at 0x7f72f0220128>]
    #+END_SRC

*** class weakref. *WeakKeyDictionary* ([dict])
    
    key为弱引用的映射类。当字典中的key没有强引用时，对应键值对会被删除。
    可用来映射应用中其他部分的数据。
    
    #+BEGIN_QUOTE
    *注意* ：WeakKeyDictionary构建在普通字典基础上。在遍历时，有可能某
    些键值对被回收，所以不能遍历所有元素。
    #+END_QUOTE
    ...
    #+BEGIN_SRC python :session
      ...
      wd = WeakKeyDictionary({foo: 1, bar: 2})
      foo = None
      print(list(wd.keys()))

      # 不使用弱引用时，即时原来作为key值的函数被删除后，依然存在于字典中
      def foo(): pass
    ...
    #+RESULTS:
    #+BEGIN_SRC org
    - >>> ... >>> ... >>> >>> >>> [<function bar at 0x7f72efb2a2f0>]
    - >>> ... ... >>> ... >>> >>> >>> [<function bar at 0x7f72efb2a400>, <function foo at 0x7f72efb2a268>]
    #+END_SRC
    
**** WeakKeyDictionary. *keyrefs* ()
     获取当前存在的弱引用key，可用来检查是否有key被回收。
     ...
     #+RESULTS:
     #+BEGIN_SRC org
     - >>> ... >>> ... >>> >>> [<weakref at 0x7f2a31754598; to 'function' at 0x7f2a3103b6a8 (foo)>, <weakref at 0x7f2a31754728; to 'function' at 0x7f2a3103b7b8 (bar)>]
     - >>> [<weakref at 0x7f2a31754728; to 'function' at 0x7f2a3103b7b8 (bar)>]
     #+END_SRC
     
*** class weakref. *WeakValueDictionary* ([dict])
**** WeakValueDictionary *valuerefs* ()
     用法同WeakKeyDictionary。
*** class weakref. *WeakSet* ([elements])
    对元素弱引用的集合类。如果某个元素上没有强引用，则在集合中被删除。
    ...
    #+RESULTS:
    #+BEGIN_SRC org
    - >>> ... >>> ... >>> >>> >>> ... ... <function bar at 0x7f72efb2a510>
    - 不使用弱引用：
    - >>> ... >>> ... >>> >>> >>> ... ... <function bar at 0x7f72efb2a488>
    - <function foo at 0x7f72efb2a2f0>
    #+END_SRC
    
*** class weakref. *WeakMethod* (method)
    定制ref子类，模仿对实例方法的弱引用。由于实例的方法会立即消失，使
    用标准的ref弱引用不会成功。使用WeakMethod返回的代理对象只有当原实
    例不存活后才没有引用。
    ...
    #+BEGIN_SRC python :session
      ...
      c = C()
      standard_ref = weakref.ref(c.method)
      method_ref = weakref.WeakMethod(c.method)
      print('对两个代理方法为：')
      print(standard_ref())
      print(method_ref())
      print('删除原实例后，仍可获取原来方法，只不过为None：')
      del c
      print(method_ref())
    #+END_SRC
    ...
    - >>> >>> ... ... ... >>> >>> >>> >>> >>> 对两个代理方法为：
    - None
    - <bound method C.method of <__main__.C object at 0x7f72efb26f98>>
    - 删除原实例后，仍可获取原来方法，只不过为None：
    - >>> None
    #+END_SRC

*** class weakref. *finalize* (obj,func,*args,**kwargs)
    返回一个可调用的finalizer对象，当被引用对象被回收后被调用。不同于
    普通弱引用，在被引用对象将要回收时调用，而是在回收后调用，从而简化
    了其生命周期管理。
    
    调用finalizer对象的返回值为FUNC(*ARGS,**KWARGS)。如果此引用对象不
    再存活，返回值为None。
    
    回调函数如果抛出错误，将显示在stderr中，不能传播。这点同实例
    的__del__()方法和弱引用对象回调函数抛出错误时的处理方式相同。
    
    除非atexit属性设置为false，每个存活的finalizer对象都会在程序结束时
    被调用。调用顺序与添加顺序相反。
    ...
    #+BEGIN_SRC python :session
      ...
      c = C()
      d = {1, 2, 3}
      f = weakref.finalize(c.method, list, '1')
      f1 = weakref.finalize(d, list, '2')
      print(f())
      print(f1())
      del d
      print(f1())
...
**** __call__()
     当自身存活时，返回调用FUNC(*ARGS,**KWARGS)调用结果，并将自身标记
     为不存活；如果自身不存活，返回None。

     #+BEGIN_SRC python :session
       import weakref
       o = set((1, 2, 3))
       f = weakref.finalize(o, list, 'abc')
       print(f.__call__())
       print(f)
       print(f.__call__())
     #+END_SRC
     ...
     #+BEGIN_SRC org
     - >>> >>> ['a', 'b', 'c']
     - <finalize object at 0x7f72f03e60b0; dead>
     - None
     #+END_SRC

**** detach()
     如果自身存活，返回元组(obj,func,*args,**kwargs)，并将自身标记为不
     存活；如果自身不存活，返回None。
     ...
     #+RESULTS:
     #+BEGIN_SRC org
     - >>> >>> ({1, 2, 3}, <class 'list'>, ('abc',), {})
     - <finalize object at 0x7f72f03e60a0; dead>
     #+END_SRC

**** peak()
     与detach()的区别为：调用后不将自身标记为不存活。
     ...
     #+BEGIN_SRC org
     - >>> >>> ({1, 2, 3}, <class 'list'>, ('abc',), {})
     - <finalize object at 0x7f72f03e60b0; for 'set' at 0x7f72f0220e48>
     - ['a', 'b', 'c']
     - <finalize object at 0x7f72f03e60b0; dead>
     #+END_SRC
...
**** atexit
     可写布尔值属性，默认值为True。当程序退出时，会调用所有此属性为
     True的finalizer对象，调用顺序与添加顺序相反。

     #+BEGIN_QUOTE
     *注意* ：需保证func、args和kwargs不对obj有直接/间接引用，否则obj
     完全没可能被回收。fun也不能为obj的实例方法。
     #+END_QUOTE
...
*** weakref. *ReferenceType*
    ...
    #+BEGIN_SRC python :session
      ...
      print(isinstance(f, weakref.ReferenceType))
    #+END_SRC

    #+RESULTS:
    #+BEGIN_SRC org
    - >>> >>> >>> <class 'weakref'> <class 'weakref.finalize'>
    - True
    - False
    #+END_SRC
...
*** weakref. *CallableProxyType*
    ...
    #+BEGIN_SRC python :session
      ...
      fp = weakref.proxy(foo)
      op = weakref.proxy(o)
      print(type(fp), type(op))
      print(isinstance(fp, weakref.CallableProxyType))
      print(isinstance(op, weakref.ProxyType))
    #+END_SRC
...
*** weakref. *ProxyTypes*
    ...
    #+BEGIN_SRC python :session
      ...
      print(isinstance(fp, weakref.ProxyTypes))
      print(isinstance(op, weakref.ProxyTypes))
    #+END_SRC
...
** Weak Reference对象
   弱引用对象除ref.__callback__外，无任何属性和方法。调用弱引用对象可
   获取原对象，如果原对象不存在，则返回None：
   ...
   如果在线程应用中使用弱引用，为保证安全无论是单线程还是多线程，使用
   被引用对象前都需检查是否已被回收，通常的模式为：
   ...
   #+BEGIN_SRC python :eval never
     ...
     if o is None:
         ...
         print('被引用对象仍存活')
         o.de_somthing_useful()
   #+END_SRC

   可通过继承ref类定制引用对象，如WeakValueDictionary。如下例实现对弱
   引用对象进行调用计数：
   ...
   #+RESULTS:
   #+BEGIN_SRC org
   - >>> ... ... ... ... ... ... ... ... ... ... ... ... >>> ... >>> >>> (<function foo at 0x7f72efb2a510>, 1)
   - foo 函数被调用
   - (<function foo at 0x7f72efb2a510>, 3)
   #+END_SRC

** 举例
   下例使用对象ID实现映射，当对象被回收后，仍可使用该对象的ID；如果未
   被回收，还可使用该ID获取原对象。
   ...
   #+BEGIN_SRC python :session
     ...
     xid = remember(x)
     print(dict(_id2obj_dict.items()))
     x = None
     # print(_id2obj_dict[xid])
     print(dict(_id2obj_dict.items()))
     print(xid)
   #+END_SRC
   ...
   #+RESULTS:
   #+BEGIN_SRC org
   - >>> >>> >>> ... ... ... ... >>> >>> ... ... >>> >>> >>> >>> >>> {140131624486952: {1, 2, 3}}
   - >>> ... {}
   - 140131624486952
   #+END_SRC

** Finalizer对象
   使用finalizer的好处为简化回调函数注册，无需保存返回的finalizer对象。
   比如：
   ...
   #+BEGIN_EXAMPLE python
     ...
     <finalize object at 0x7f4558c1f0a0; for 'Object' at 0x7f4558a69198>
     >>> del kenny
     You killed Kenny
   #+END_EXAMPLE
   
   finalizer对象可直接调用。调用后自身不再存活：
   ...
   #+BEGIN_EXAMPLE python
     ...
     回调函数被调用
     >>> assert not f.alive
     >>> f()     # 由于finalizer对象不再存活，回调函数不会被调用
     >>> del obj # 由于finalizer对象不再存活，回调函数不会被调用
   #+END_EXAMPLE
   
   可使用detach()方法返回构建弱引用对象时的参数，并使finalizer对象不再
   存活：
   ...
   #+BEGIN_EXAMPLE python
     ...
     >>> f = weakref.finalize(obj, callback, 1, 2, z=3)
     >>> f.detach()
     (<__main__.Object object at 0x7f4558a69160>, <function callback at 0x7f4558a6a0)
   ...
   如果atexit属性不为False，如果finalizer对象存活，回调函数在程序退出
   时也会调用：

   #+BEGIN_EXAMPLE python
     >>> obj = Object()
     回调函数被调用
     >>> f = weakref.finalize(obj, print, '对象被回收或程序退出')
     >>> exit()
     对象被回收或程序退出
   #+END_EXAMPLE

** finalizers与__del__方法
   假设创建类表示临时文件夹，文件夹及其内容需在下面任何一种情况发生时
   被删除：
   ...
   从Python 3.4开始，__del__()方法调用时，即时环形引用的也会被回
   收，在interpreter shutdown时，模块的全局变量不被强制为None。所以此
   方法在CPython中没有问题。？？？

   但是，__del__()依赖实现的垃圾回收机制实现，所以更健壮的方法时使用弱
   引用：
   ...
   #+BEGIN_SRC python :session
     ...
     class TempDir:
         def __init__(self):
             self.name = tempfile.mkdtemp()
             self.__finalizer = weakref.finalize(self, shutil.rmtree, self.name)
         def remove(self):
             self.__finalizer()
         @property
         def removed(self):
...
* types - 内置类型动态创建和命名 （待续）
* copy - 浅层复制和深层复制 （待续）
* pprint - 数据美化显示 （待续）
* reprlib - 另一个repr()实现 （待续）
* enum - 枚举支持 （待续）
