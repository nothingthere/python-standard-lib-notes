...
** 简介
   ...
   multiprocessing模块还实现了threading模块没有的API，其中最主要的为
   Pool类，可将数据分发到多个进程中，同步执行输入数据。如：
...
*** 上下文和启动方法
    ...
    - spawn：
      ...
      run()方法的资源。不继承不需要的文件描述符和处理器。较后面两种方
      法相当慢。
...
**** 管道
     ...
     等方法。如果多个进程（或线程）尝试同时在管道的同一段读/写数据，可
     能会造成数据毁坏；同时读/写不同断则不会。
...
***** close()
***** poll([timeout])
***** send_bytes([maxlength])
***** recv_bytes_inti(buffer[,offset])
*** 同步原始
*** 共享C类型库
