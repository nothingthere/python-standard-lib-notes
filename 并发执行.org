...
** ProcessPoolExecuter
   ProcessPoolExecuter为Executor的子类，使用进程池实现异步调用。
   ProcessPoolExecuter使用multiprocessing模块，所以可避开“全局解释器锁”，
   但也是只能执行和返回picklable的对象。

   工作子进程需可导入__main__模块，也就意味着ProcessPoolExecuter不能在
   交互式解释器中运行。

   在向ProcessPoolExecuter提价的任务中调用Executor或Future会造成锁死。

*** class concurrent.futures. *ProcessPoolExecutor* (max_workers=None)
    Executor的子类，使用数量最多为max_workers的进程组成的进程池实现同
    步调用。

    max_worker：默认为计算机处理器数量；如果<=0，抛出ValueError错误。

    如果某个工作进程“唐突”终止，会抛出BrokenProcessPool错误。
    ...
    #+BEGIN_EXAMPLE python
      ...
      def timeit(fn):
          ...
          def foo(*args, **kwds):
              s_t = time.time()
              fn(*args, **kwds)
              print('耗时：{}'.format(time.time() - s_t))
              print('-' * 20)
          return foo

      @timeit
      def main_with_no_concurrent():
          for num in PRIMES:
              print('{} 是否为质数：{}'.format(num, is_prime(num)))

      @timeit
      def main():
          with concurrent.futures.ProcessPoolExecutor() as executor:
              for num, prime in zip(PRIMES, executor.map(is_prime, PRIMES)):
                  print('{} 是否为质数：{}'.format(num, prime))

      if __name__ == '__main__':
          main_with_no_concurrent()
          main()
    #+END_EXAMPLE

** Future对象
   由Executor.submit()创建，封装异步调用执行。

*** class concurrent.futures. *Future*
    封装异步调用执行，由Executor.submit()创建，除测试外不得直接创建。

**** 普通方法
***** cancel()
      取消执行调用。如果当前正在执行，且不能被取消此函数返回False，否
      则取消执行且返回True。
...
***** running()
      如果正在执行，且不能被取消则返回True。

***** done()
      如果被成功取消，或完成执行则返回True。

***** result(timeout=None)
      返回执行结果。如果调用还没完成，则此方法等待最多timeout指定秒数。
      如果在timeout指定时长内还没完成，抛出
      concurrent.futures.TimeoutError错误。如果不指定timeout参数或为
      None（默认），则可无限期等待。

      如果执行完成前被取消，调用此方法会抛出CancelledError错误。

      如果子进程中执行调用时抛出错误，会被此方法重新抛出。

***** exception(timeout=None)
      返回子进程中执行时抛出的错误。如果还没完成执行，则等待最多
      timeout指定秒数。如果在timeout指定时长内还没完成执行，则抛出
      concurrent.futures.TimeoutError错误。如果不指定timeout参数或为
      None（默认），则可无限期等待。

      如果执行完成前被取消，调用此方法会抛出CancelledError错误。

      如果执行完毕且无错误抛出，则返回None。

***** add_done_callback(fn)
      为future对象添加回调函数fn，接受future对象作为唯一参数。在future
      对象取消后或执行完毕后调用。

      可添加多个回调函数，执行顺序与添加顺序相同，执行环境为添加回调函
      数所在进程中的一个线程中。如果回调函数抛出Exception子类错误，会
      被记录并忽视；如果抛出BaseException子类错误，行为还没被定义。

      如果future已经执行完成或被取消，则fn函数立即被调用。

**** 用于单元测试和Executor实现的方法
***** set_running_or_notify_cancel()
      此方法仅可在Future执行前被Executor实现调用，或被单元测试调用。

      如果返回False，则取消Future的执行，如调用Future.cancel()会返回
      True。任何等待Future完成的线程（如通过wait()或as_completed()）都
      会被唤醒。

      如果此方法返回True，则不取消Future执行，并将其置于运行状态，如调
      用Future.running()会返回True。

      此方法仅能调用一次，且不能在Future.set_result()或
      Future.set_exception()后调用。
...
***** set_exception(exception)
      设置Future抛出错误exception。

      只能用于Executor实现和单元测试。

** 模块函数
*** class concurrent.futures. *wait* (fs,timeout=None,return_when=ALL_COMPLETED)
    等待fs指定的多个Future实例（可能由不同Executor实例创建）执行完成。
    返回由2个set元素组成的命名元组。第一个set名为“done”，含所有完成的
    futures（取消或执行完成）；第二个set名为"not_done"，含所有未完成的
    futures。

    timeout：指定返回前最多的等待秒数。如果不指定或为None（默认），则
    可无限期等待。
    ...
    #+BEGIN_EXAMPLE python
      ...
      if __name__ == '__main__':
          ...
          future = e.submit(f)
          print(wait([future], timeout=0.5))
    #+END_EXAMPLE

    return_when：指定此函数何时返回，需为下面的模块常量：

    | 常量            | 含义                                            |
    |-----------------+-------------------------------------------------|
    | FIRST_COMPLETED | 当任何future完成执行或被取消时返回              |
    | ALL_COMPLETED   | 当所有future完成执行或被取消时返回              |
    | FIRST_EXCEPTION | 当任何future因为有错误抛出执行完成时返回，      |
    |                 | 如果没有任何future抛出错误，相当于ALL_COMPLETED |

*** class concurrent.futures. *as_completed* (fs,timeout=None)
    返回迭代对象，其__next__方法返回fs（可能由不同Executor实例创建）中
    完成（执行完成或被取消）的future对象。如果fs中有重复，则只返回其中
    一个。fs中任何在调用as_completed前就完成的future会被先返回。

    timeout：如果自调用as_completed()方法时算起，对返回对象调
    用__next__方法不能在timeout指定时长后获取返回值，则抛出
    concurrent.futures.TimeoutError错误。如果不指定或为None（默认），
    可无限期等待。
...
*** exception concurrent.futures. *TimeoutError*
    当对future对象的操作超过指定时长时抛出。

*** exception concurrent.futures. *BrokenProcessPool*
    源自RuntimeError，当ProcessPoolExecutor中的工作进程被“唐突”终止
    （如被外部环境杀死）时抛出。
...
* sched - 事件调度
  ...
  [[https://lz5z.com/Python%25E5%25AE%259A%25E6%2597%25B6%25E4%25BB%25BB%25E5%258A%25A1%25E7%259A%2584%25E5%25AE%259E%25E7%258E%25B0%25E6%2596%25B9%25E5%25BC%258F/][@LeoScript]]

  此模块定义了一个实现通用事件调度的类：scheduler。

** class sched. *scheduler* (timefunc=time.monotonic,delayfun=time.sleep)
   scheduler类定义了调度事件的通用接口。需两个函数与“外部世界”产生联系：

   - timefunc：无参数，返回数值（表示任何单位的“时间”）。如果Python实
     现中无time.monotonic，则默认使用time.time。
   - delayfunc：需1个参数，与timefunc返回值兼容，延迟timefunc返回值指
     定时长。当每个事件执行完毕后，会使用0作为参数调用一次，使多线程程
     序中其他线程有机会运行。
...
*** 举例
    ...
    再如实现类似JavaScript的setInterval功能：
...
*** scheduler. *enterabs* (time,priority,action,argument=(),kwargs={})
    ...
    time：须为数字类型，且与timefunc返回值兼容。如果将多个事件调用为同
    一时间执行，则优先级（通过priority参数设置）越高的事件先执行。

    事件执行相当于action(*argument, **kwargs)。其中argument为位置参数
    组成的链表，kwargs为关键字参数组成的字典。

    返回值为事件对象，可用于被cancel()方法取消。

*** scheduler. *enter* (delay,priority,action,argument=(),kwargs={})
    调度时间使其延迟delay参数指定时长执行。与enterabs()不同，时间为相
    当时间；其他用法与enterabs()完全相同。

*** scheduler. *cancel* (event)
    移除队列中的事件event，如果event不在当前事件队列中，抛出ValueError
    错误。
...
*** scheduler. *run* (blocking=True)
    运行所有已被调度的事件。使用构造器中delayfunc()等待下一个事件，执
    行等到的事件，直到所有被调度的事件执行完毕。

    blocking：If blocking is false executes the scheduled events due
    to expire soonest (if any) and then return the deadline of the
    next scheduled call in the scheduler (if any).？？？

    action和delayfunc都可抛出错误。此两种情况下，调度器会维持连续状态，
    并向上传播抛出错误。如果错误由action抛出，则该事件不会在以后调用
    run()时执行。？？？

    如果某个事件任务运行事件多于可用时长，调度器会在后面执行。不会落下
    任何事件，调用代码有责任取消不适宜事件。？？？

*** scheduler. *queue*
    只读属性，按顺序返回即将执行事件组成的链表。每个时间以命名元组形式
    返回，有域：time、priority、action、argument和kwargs。
